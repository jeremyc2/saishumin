...
export const autoPanCamera = ({
	...
	const horizontalVelocity = axisAutoPanSpeed(pointer.x, viewport.width);
	const verticalVelocity = axisAutoPanSpeed(pointer.y, viewport.height);
	return {
...
export const contentEnvelope = (world: World): ScreenBounds => {
	...
	for (const [entity, position] of world.positions) {
		...
		const top = base + entityHeight(world, entity);
		envelope = unionBounds(
			envelope,
			boundsForPoints([
				...projectedRectangle(position, body, base),
				...projectedRectangle(position, body, top),
			]),
		);
	}
	return envelope;
