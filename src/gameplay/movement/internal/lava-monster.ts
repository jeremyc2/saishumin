...
export const updateLavaMonster = (world: World, elapsed: number): World => {
	...
	if (
		...
		return world;
	if (!canPlaceLavaMonster(world, position, elevation.z)) {
	...
	const shouldJump =
		grounded &&
		lavaMonsterNeedsJump(world, position, elevation.z, playerPosition);
	...
	const moved = canPlaceLavaMonster(world, verticalCandidate, elevation.z)
		? verticalCandidate
		: afterHorizontal;
	let velocity =
