...
type ActiveInteraction =
	| { readonly kind: "pan"; readonly pointer: Position; readonly camera: Position }
	| {
			readonly kind: "move";
...
const minimumEntityBody = { width: minimumEntityExtent, depth: minimumEntityExtent } as const;
const minimumFloorBody = { width: minimumFloorWidth, depth: minimumFloorDepth } as const;
const maximumFloorBody = { width: Number.POSITIVE_INFINITY, depth: Number.POSITIVE_INFINITY } as const;
const wheelLinePixels = 16;

export type DesignStudioInteractionRuntime = {
	readonly startPan: (event: PointerEvent, world: World) => void;
	readonly startEntityMove: (event: PointerEvent, world: World, entity: EntityId, dispatch: Dispatch) => void;
	readonly startEntityResize: (event: PointerEvent, world: World, entity: EntityId, widthDirection: ResizeDirection, depthDirection: ResizeDirection, dispatch: Dispatch) => void;
	readonly startFloorResize: (event: PointerEvent, world: World, widthDirection: ResizeDirection, depthDirection: ResizeDirection, dispatch: Dispatch) => void;
	readonly startPaletteDrag: (event: PointerEvent, itemKind: EditorItemKind, world: World) => void;
	...
	readonly palettePopover: () => { readonly left: number; readonly top: number; readonly fading: boolean } | null;
};
	...
	Effect.gen(function* () {
		...
		yield* Effect.acquireRelease(
			...
			() => Effect.sync(() => {
				...
				if (autoPanAnimationFrame !== undefined) cancelAnimationFrame(autoPanAnimationFrame);
				if (popoverAnimationFrame !== undefined) cancelAnimationFrame(popoverAnimationFrame);
				activeInteraction = undefined;
		...
		return {
			...
			isPanGesture: (event) => event.button === 1 || event.metaKey || event.ctrlKey,
			isGestureActive: () =>
