	...
	static readonly layer = Layer.effect(this)(
		Effect.gen(function* () {
			...
			const renderWorld = (
				...
				const designStudioViewForRender = designStudioView;
				if (designStudioViewForRender === undefined) return;
					...
					<main class=${`relative h-screen w-screen overflow-hidden bg-[#14212a] ${interactionRuntime.isGestureActive() ? "editor-active-gesture" : ""} ${invalidPreview ? "editor-invalid-preview-root" : ""}`} @pointerdown=${(
						...
						>
							...
							<g transform=${`translate(${camera.x} ${camera.y})`}>
								...
								${world.editor.open ? designStudioViewForRender.selectionTemplate(world, invalidPreview, dispatch) : svg``}
							</g>
						</svg>
						...
						${
							world.editor.open
								? html`<div class="pointer-events-none absolute bottom-6 left-7 rounded-xl bg-[#0d181f]/88 px-4 py-2 text-[11px] font-bold tracking-[0.12em] text-[#e8b875]">CONTROLS PAUSED · INFINITE PAN · FIXED SCALE</div>${designStudioViewForRender.editorPanelTemplate(world, !presentation.active, dispatch)}`
								: html`
						...
						${designStudioViewForRender.paletteGuidanceTemplate(palettePopover)}
						${world.editor.invalidPlacement === null && !presentation.invalidReleased ? html`` : designStudioViewForRender.invalidPlacementTemplate(world, presentation, dispatch)}
						${world.readingSign === null ? html`` : designStudioViewForRender.signDialogTemplate(world, dispatch)}
					</main>
				`,
					document.body,
				);
			...
			const refreshCreatePreview = (): void => {
				...
				const designStudioViewForPreview = designStudioView;
				const host = document.querySelector("#editor-create-preview-host");
				if (
					world === undefined ||
					designStudioViewForPreview === undefined ||
				...
				render(
					preview === null
						? html``
						: designStudioViewForPreview.createPreviewTemplate(
								world,
